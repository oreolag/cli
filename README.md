...
```odev``` is the CLI for heterogeneous computing. Building on the Heterogeneous Accelerated Compute Cluster CLI developed at ETH Zurich (see hdev on GitHub), ```odev``` abstracts NUMA-local heterogeneous resources as topology-aware computing units that simplify the deployment and execution of real-world AI, HPC, and ML workloads across modern accelerated computing infrastructures. Through integrations with popular workflows and frameworks such as NCCL, Metaflow, Slurm, and Kubernetes, ```odev``` enables reproducible execution across CPUs, GPUs, FPGAs, storage, and high-performance networking systems.
